<?php
function processGETRequest($appkey, $token, $text, $audioSaveFile, $format, $sampleRate,$voice,$volume,$speech_rate) {
...
$appkey = "UfC9sGADBDjwBcnE";
$text = isset($_POST['text'])?$_POST['text']:'';
if(!$text){print 0;exit;}
...
$voice=isset($_POST['voice'])?$_POST['voice']:'xiaoyun';
$volume = isset($_POST['volume'])?$_POST['volume']:'50';
$speech_rate = isset($_POST['speech_rate'])?$_POST['speech_rate']:'0';

// $filename = 'syAudio';
$filename = isset($_POST['filename'])?$_POST['filename']:'syAudio';
$audioSaveFile = $filename.".mp3";
$format = "mp3";
$sampleRate = 16000;

$token_file=json_decode(file_get_contents('./token'),1);
if(!isset($token_file['Token']['Id']) || $token_file['Token']['ExpireTime']<time()){
    include './get_token.php';
    
    // $token_file=json_decode(get_token(),1);
    get_token();
    $token_file=json_decode(file_get_contents('./token'),1);
}
$token=$token_file['Token']['Id'];


processGETRequest($appkey, $token, $textUrlEncode, $audioSaveFile, $format, $sampleRate,$voice,$volume,$speech_rate);
